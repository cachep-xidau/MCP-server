...
### 2.2.1. VPS Environment (Cloud Data Ingestor)
The VPS sub-system serves as the Heavy-lifting Node. It automatically extracts and transforms raw data sources into a Vector database.

- **Cronjob Scheduler:** The system scheduler (cron) is responsible for triggering the Extract, Transform, Load (ETL) pipeline at `1:00 AM` daily, optimizing bandwidth during off-peak hours.
- **Python Ingestor Script:** The extraction module interfaces directly with the **Atlassian API** to aggregate documents (PRDs, Epics, User Stories) from Jira and Confluence platforms. The data then undergoes preprocessing, including sanitization and chunking.
- **HuggingFace Embedding Model:** The Natural Language Processing (NLP) analysis layer. The process utilizes the highly optimized, multilingual `BAAI/bge-m3` embedding model to encode and convert text into a Vector Semantic Space.
- **ChromaDB Vector Database:** The primary Vector database storing the output. At this stage, ChromaDB operates strictly in a Write-Optimized role to maintain the integrity of the original data.

### 2.2.2. Local Environment (macOS Edge Node)
The Local sub-system transforms the user's endpoint into an inference edge node, ensuring absolute Data Privacy and Zero-Latency queries.

- **macOS LaunchAgent:** A background daemon configured to automate the synchronization process. Every 4 hours, the system triggers the `rsync` protocol to download differential files (Delta changes) of the Vector Database from the VPS server to the local workstation.
- **ChromaDB Local Mirror:** A completely independent local replica of the database. This architecture eliminates remote Network I/O, maximizing context retrieval speed.
- **Python FastMCP Server:** The core communication interface at the endpoint, adhering to the Model Context Protocol (MCP) standard.
    - Instead of transmitting user queries to a central server, the query vectorization process generated by the LLM is computed entirely on the local Mac using the `HuggingFace (BAAI/bge-m3)` module.
    - It executes a Similarity Search against the **Local Mirror DB** to extract relevant context.
- **LLM Client:** Ad-hoc AI Integrations (e.g., Cursor AI, Claude Desktop). The application directly hooks into the Context Tool provided by FastMCP, executing business logic analysis natively on the local processor, ensuring zero leakage of enterprise data constructs to external networks.

### 2.2.3. Core Benefits of the Architecture
- **Cost & Performance Optimization:** Complex computational overhead (Noise reduction, Embedding API costs) is completely offloaded to the VPS.
- **Maximum Privacy:** The Edge Inference pipeline empowers the LLM to query the Atlassian database without exposing internal Business Security, Trade Secrets, or sensitive artifacts to the public Cloud, solidifying a sustainable security loop.
