...
The server is a **single Node.js/TypeScript process** that runs on the **VPS** (single source of truth) and speaks MCP over stdio. There is no embedding model and no vector database in the current implementation — retrieval is lexical BM25. Semantic vector search is a possible future upgrade.
...
C4Container
    ...
    Person(user, "BA / User", "Asks questions via an AI agent")
    ...
    System_Ext(ingest, "Ingestion Job", "Cron, external")

    System_Boundary(vps, "VPS") {
        Container(mcp, "DON-Workspace-MCP", "Node.js / TypeScript, stdio", "MCP tools; RBAC/ACL enforced")
        ContainerDb(db, "remote-rag.db", "SQLite FTS5/BM25", "pages + pages_fts")
    }

    Rel(user, agent, "Chats")
    Rel(agent, mcp, "Invokes tools", "MCP stdio / Tailscale SSH")
    Rel(mcp, db, "ACL-scoped BM25 read", "readonly")
    Rel(mcp, atlassian, "Live REST", "HTTPS")
    Rel(mcp, figma, "Live REST", "HTTPS")
    Rel(ingest, atlassian, "Scrapes", "HTTPS")
    Rel(ingest, db, "Builds / updates", "cron")
```
...
- **Store:** SQLite `remote-rag.db`, FTS5 table `pages_fts` over base `pages` (`id, title, url, project, ...`), opened read-only.
- **Ranking:** BM25, top-5; OR-synonym expansion for recall.
- **ACL pre-filter:** `resolveAclScope()` adds `WHERE p.project IN (...)`; out-of-scope requests denied.

### 2.2.3 Access Control (RBAC / ACL)
Provisioned per deployment via env vars:
- `RBAC_ROLE` — caller role (reserved for role-aware gating).
- `ACL_ALLOWED_PROJECTS` — permitted project keys; unset = unrestricted.

### 2.2.4 Design Note — VPS-only
Earlier edge mirror (`sync-db.sh` rsync every 4h) removed — caused up to 4h staleness for negligible latency gain. Server now reads `remote-rag.db` in place; clients connect over Tailscale SSH stdio.
...
sequenceDiagram
    ...
    Note over Cron: Sanitize + upsert rows into 'pages'
    Cron->>DB: Rebuild FTS5 index 'pages_fts'
```
> The ingestion job is operated on the VPS and is **not part of this repository**.
...
## 5. Performance Evaluation: RAG + Workflow vs. Direct Search
...
| **Fuzzy / Paraphrased Queries** | Low. Misses without exact keywords. | **Medium.** BM25 + synonym expansion helps; full semantic recall is a possible future upgrade. |
